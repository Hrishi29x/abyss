...
{ 
  ...
  if(sy == gy)
  {
    box.w = fabs((sx - gx) * 1.0);
    box.h = 1;                                        
    SDL_FillRect(screen,&box,color);    
    return;
  }
  box.h = fabs((sy - gy) * 1.0);
  box.w = 1;                                        
  SDL_FillRect(screen,&box,color);    
}
...
void DrawThickLine(int sx,int sy,int dx, int dy,int width,Uint32 Color,SDL_Surface *surface)
{
  ...
  box.w = width;
  box.h = width;
  deltax = fabs((dx - sx) * 1.0);        // The difference between the x's
  deltay = fabs((dy - sy) * 1.0);        // The difference between the y's
